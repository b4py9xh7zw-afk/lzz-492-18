...
### 4. 高温作业保障模块 🌡️
- **高温策略配置**: 可配置高温阈值（默认35℃）、津贴标准（元/人/天）、休息频次说明、防暑物资发放量（盐丸/冰袖/饮水券）
- **排班自动提示**: 室外岗位气温达到阈值时，排班页自动高亮并提示高温津贴、休息频次与物资发放要求；新增/编辑排班时实时预警
- **物资自动下单**: 高温排班保存后自动生成"待发放"物资记录，无需人工登记
- **仓管确认发放**: 仓管在PC端确认盐丸、冰袖、饮水券发出，记录确认人与确认时间
- **工人手机端可查**: 工人在H5端按姓名查询自己的排班高温提示与物资发放记录
- **结算分列展示**: 结算单将"普通工时工资"与"高温津贴"分组列示（工时/时薪/工时工资 | 高温天数/津贴标准/津贴合计），工资明细清晰；支持按工人+月份生成、预览、结算，同周期幂等防重
...
#### 高温作业保障
- `GET /api/heat-policy/active` - 获取当前启用的高温策略
- `PUT /api/heat-policy/{id}` - 更新高温策略（阈值/津贴/休息频次/物资量）
- `POST /api/schedule` - 新增排班（触发高温保障时自动生成物资发放单）
- `PUT /api/schedule/{id}` - 更新排班（同步物资发放单）
- `DELETE /api/schedule/{id}` - 删除排班
- `GET /api/schedule/page` - 分页查询排班（附带高温津贴、休息频次、物资提示）
- `GET /api/supply/page` - 分页查询物资发放记录（工人手机端按姓名查询）
- `PUT /api/supply/{id}/confirm` - 仓管确认发放
- `GET /api/settlement/page` - 分页查询结算单
- `GET /api/settlement/preview` - 预览结算（津贴与普通工时分列，不落库）
- `POST /api/settlement/generate` - 生成结算单（同工人同周期幂等）
- `PUT /api/settlement/{id}/settle` - 标记已结算
